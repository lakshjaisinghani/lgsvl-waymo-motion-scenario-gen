...
if __name__ == "__main__":
    ...
    forward = lgsvl.utils.transform_to_forward(ego_spawn)
    ego_spawn.transform.position += 5 * forward  # 5m forwards
    ...
    npc_sim = sim.add_agent("SchoolBus", npc.agent_type['lgsvl'], npc.spawn)
    ...
    #enable kitti parsing
    kitti = KittiParser(ego=ego, sim=sim)
    kitti.bootstrap()

    input("Press Enter to run the simulation for 20 seconds")
